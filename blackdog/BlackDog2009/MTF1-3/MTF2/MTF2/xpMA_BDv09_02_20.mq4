//+------------------------------------------------------------------+
//|                                             xpMA BDv09_02_20.mq4 |
//|                                                             Jeff |
//|                                                  wolfboy@att.net |
//+------------------------------------------------------------------+
#property link      "wolfboy@att.net"

// xpMA BDv8_12_31:   Original indicator below (xpMA) was modified for the Black Dog Trading System.
// xpMA BDv09_01_02:  Fixed up arrow vertical line problem when indicator is applied to certain pairs.
//                    If you are using an EA that calls the xpMA, DO NOT use v09_01_02 or later for
//                    the EA.
// xpMA BDv09_02_20:  Adds options for where to place the xpMA Black Dog Arrow Direction comment
//                    and other comment options.  BDTextWhatCorner: 0=upper left; 1=upper right; 
...
//               signal[shift]=-1;
   ...
   string xpMABDUpText = "Latest xpMA Black Dog Arrow is:  UP";
   string xpMABDDnText = "Latest xpMA Black Dog Arrow is:  DOWN";
   string xpMABDFlatText = "Latest xpMA Black Dog Arrow is:  FLAT";
   
   if(UpBuffer[shift] != EMPTY_VALUE)
      {
      ObjectCreate("xpMA_Direction_Text",OBJ_LABEL,0,0,0);
      ObjectSetText("xpMA_Direction_Text",xpMABDUpText,CommentFontSize,CommentFontType,CommentColor);
      ObjectSet("xpMA_Direction_Text",OBJPROP_CORNER,CommentCorner);
      ObjectSet("xpMA_Direction_Text",OBJPROP_XDISTANCE,CommentXoffset);
      ObjectSet("xpMA_Direction_Text",OBJPROP_YDISTANCE,CommentYoffset);
      }
   if(DownBuffer[shift] != EMPTY_VALUE)
      {
      ObjectCreate("xpMA_Direction_Text",OBJ_LABEL,0,0,0);
      ObjectSetText("xpMA_Direction_Text",xpMABDDnText,CommentFontSize,CommentFontType,CommentColor);
      ObjectSet("xpMA_Direction_Text",OBJPROP_CORNER,CommentCorner);
      ObjectSet("xpMA_Direction_Text",OBJPROP_XDISTANCE,CommentXoffset);
      ObjectSet("xpMA_Direction_Text",OBJPROP_YDISTANCE,CommentYoffset);
      } 
   if(UpBuffer[shift] == EMPTY_VALUE && DownBuffer[shift] == EMPTY_VALUE)
      {
      ObjectCreate("xpMA_Direction_Text",OBJ_LABEL,0,0,0);
      ObjectSetText("xpMA_Direction_Text",xpMABDFlatText,CommentFontSize,CommentFontType,CommentColor);
      ObjectSet("xpMA_Direction_Text",OBJPROP_CORNER,CommentCorner);
      ObjectSet("xpMA_Direction_Text",OBJPROP_XDISTANCE,CommentXoffset);
      ObjectSet("xpMA_Direction_Text",OBJPROP_YDISTANCE,CommentYoffset);
      }
